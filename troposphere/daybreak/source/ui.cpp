...
namespace dbk {
    ...
    void FileMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "选择固件位置", x, y, WindowWidth, WindowHeight);
    ...
    bool InitializeMenu(u32 screen_width, u32 screen_height) {
        ...
        if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereApiVersionConfigItem), &version))) {
            ChangeMenu(std::make_shared<ErrorMenu>("未找到大气层", "Daybreak需要安装大气层。", rc));
            return false;
        }
        ...
        if (!ams_supports_sysupdate_api) {
            ChangeMenu(std::make_shared<ErrorMenu>("大气层版本过旧", "Daybreak需要大气层0.14.0或更高版本。", rc));
            return false;
        }
